package com.almeja.pel.gestao.infra.service.file;
...
import com.almeja.pel.gestao.infra.service.aws.s3.S3StorageService;
...
public class FileHandlerService implements FileHandlerGTW {
    ...
    public FileUploadedRecord uploadFile(MultipartDTO multipartDTO) {
        ...
        try {
            if (s3Enabled) {
                ...
                return new FileUploadedRecord(uploadToLocal(multipartDTO.getBytes(), multipartDTO.getFilename()), fileSize, false);
            }
        } catch (Exception e) {
    ...
    @Override
    public FileUploadedRecord uploadFile(byte[] fileBytes, String filename) {
        // Validar parâmetros
        validateFileParameters(fileBytes, filename);

        BigDecimal fileSize = BigDecimal.valueOf(fileBytes.length);

        try {
            if (s3Enabled) {
                log.info("Upload via S3 habilitado - enviando arquivo para bucket");
                return new FileUploadedRecord(s3StorageService.upload(fileBytes, filename, false), fileSize, true);
            } else {
                log.info("Upload local habilitado - salvando arquivo em: {}", uploadDir);
                return new FileUploadedRecord(uploadToLocal(fileBytes, filename), fileSize, false);
            }
        } catch (Exception e) {
    ...
    private String uploadToLocal(byte[] fileBytes, String filename) {
        try {
            ...
            log.error("Erro ao salvar arquivo localmente: {}", filename, e);
    ...
    private void validateFileParameters(byte[] fileBytes, String filename) {
        ...
        // Validar tamanho do arquivo
        validateFileSizeFromBytes(fileBytes);

        // Validar extensão do arquivo
        String filenameToValidate = filename.toLowerCase();
        if (!isValidFileExtension(filenameToValidate)) {
            throw new ValidatorException("Tipo de arquivo não permitido");
        }
    }

    private void validateFileSizeFromBytes(byte[] fileBytes) {
